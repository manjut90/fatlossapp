      ...
      >
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          <Animated.View style={{ flex: 1, justifyContent: 'center', paddingHorizontal: 28, paddingBottom: 24, gap: 10, opacity: formO }}>
            <Text style={{ fontSize: 32, fontWeight: '900', color: '#F7F8FC' }}>Welcome Back</Text>
            <Text style={{ fontSize: 15, color: '#6B7280' }}>Your comeback starts now. </Text>
            ...
            <TextInput placeholder="Password" placeholderTextColor="#6B7280" secureTextEntry value={password} onChangeText={setPassword} onFocus={() => setFocused('pass')} onBlur={() => setFocused('')} style={[s.input, focused === 'pass' && s.inputFocused]} returnKeyType="done" onSubmitEditing={login} />
            
            <TouchableOpacity onPress={() => {}}>
              <Text style={{ color: '#8B7CFF', fontWeight: '600', fontSize: 13, textAlign: 'right' }}>Forgot Password?</Text>
            </TouchableOpacity>

            <Animated.View style={{ transform: [{ scale: btn }] }}>
              <TouchableOpacity onPress={login} disabled={loading} style={{ borderRadius: 18, overflow: 'hidden' }}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Signup')}>
              <Text style={s.link}>Don't have an account? <Text style={{ color: '#8B7CFF', fontWeight: '800' }}>Sign Up</Text></Text>
            </TouchableOpacity>
          </Animated.View>
        </View>
